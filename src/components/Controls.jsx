const Controls = ({ validN, setN, speedMs, setSpeedMs, running, onStart, onTogglePause, paused, onReset }) => {
  return (
    <section className="bg-white/5 border border-white/10 rounded-2xl p-5 sm:p-6 shadow-xl shadow-black/20">
      <form className="grid gap-4 sm:grid-cols-3 items-end" onSubmit={(e) => { e.preventDefault(); if (!running) onStart(); }}>
        <div>
        ...
        <div className="flex gap-2">
          <button type="submit" disabled={running} className={`flex-1 rounded-lg px-4 py-2 font-medium ${running ? 'bg-blue-600/50 cursor-not-allowed' : 'bg-blue-600 hover:bg-blue-500 active:bg-blue-700'}`}>Start</button>
